	...
	for (int row = 0; row < 5; row++) {
		for (int col = 0; col < 6; col++) {
			...
			case ' ':
				...
				break;
			case 'Y':
				anotherC.writeToBuffer(4 + col, 5 + row, "л", 0xEE);
				break;
			case 'G':
				anotherC.writeToBuffer(4 + col, 5 + row, "л", 0x66);
				break;
			}
		}
	}
	...
	for (int row = 4; row < 20; row++)
	{
		for (int col = 0; col < 22; col++)
		{
			if (map.getvolcanoI() == true)
			{
				...
				{
					anotherC.writeToBuffer(137 + col, row, "л", 0x6E);
				}
				else if ((disasterindicator[row][col] == 'R'))
				{
					anotherC.writeToBuffer(137 + col, row, "л", 0x4C);
				}
				else
				{
					anotherC.writeToBuffer(137 + col, row, " ", 0x7F);
	...
	for (int row = 20; row < 38; row++)
	{
		for (int col = 0; col < 22; col++)
		{
			if (map.gettornadoI() == true)
			{
				...
				{
					anotherC.writeToBuffer(137 + col, row, "л", 0x7F);
				}
				else
				{
					anotherC.writeToBuffer(137 + col, row, " ", 0x7F);
